# 小红书笔记格式指南

## 目的

将微信公众号版的完整内容转换为适合小红书笔记的纯文本格式。用户可以直接复制粘贴到小红书编辑器中发布。

## 设计风格

**温暖、干净、手机优先**——模拟小红书 App 的阅读体验。段落简短，行距宽松，适合拇指滑动阅读。不使用 emoji，保持文字本身的质感。

## 格式规范

小红书笔记编辑器支持的格式：
- 加粗文字（编辑器内选中文字 → 点击"B"按钮，**但这是编辑器操作，不是 Markdown**）
- 换行分段
- 表情符号（但本指南不使用）

不支持的格式：
- HTML 标签
- **Markdown 语法**（包括 `**加粗**`、`*斜体*`、`# 标题` 等——输出文本中的 `**` 会原样显示为星号）
- 自定义字体/字号
- 标题层级（无 H1/H2）

## 纯文本模板

```
{{文章标题}}

{{YOUR_NAME}} · {{YYYY.MM.DD}}

——————

{{导语或第一段，直接进入正文}}

「{{小标题1}}」

正文段落...

「重点内容用书名号标记，不用 ** 加粗」

「{{小标题2}}」

正文段落...

引用内容可以用竖线标记：
｜引用文字放在这里
｜多行引用也可以

「{{小标题3}}」

正文段落...

——————

{{结尾金句}}

如果你对此有任何感想，欢迎在评论区聊聊
```

## 格式标记说明

小红书不支持 Markdown 语法，输出文本中不得出现 `**文字**`、`*文字*` 等 Markdown 标记（会原样显示为星号）。强调方式：

- **小标题**：用「」包裹，如 `「这是小标题」`
- **重点文字**：同样用「」包裹，如 `「核心观点」`。禁止使用 `**文字**`
- **引用**：用全角竖线 `｜` 开头，视觉上模拟引用效果
- **分隔线**：用 `——————`（六个全角破折号）
- **不使用 emoji**
- **不使用话题标签**（标签由用户自己在小红书编辑器中添加）

## 内容转换规则

使用微信公众号版的完整内容，转换时：

1. 标题直接使用公众号版标题
2. 不需要导语区分——直接进入正文
3. 公众号版的编号色块小标题 → 用「」包裹的文字
4. 公众号版的加粗/荧光笔 → 用「」包裹（**禁止用 `**文字**`**）
5. 公众号版的引用块 → 用 `｜` 开头的引用行
6. 公众号版的结尾金句 → 单独一行，用「」包裹
7. 不使用编号（01、02...），小标题用「」即可
8. 段落保持简短，每段 2-4 句

## 输出

- 文件格式：`.txt`
- 文件名：`[主题关键词]_小红书.txt`
- 保存到：`/mnt/user-data/outputs/`

## 重要提醒

- **内容与公众号版完全一致**——只改格式，不改文字
- **纯文本输出**——不使用 HTML
- **不使用 emoji**——保持文字质感
- **不使用话题标签**——由用户自行添加
- **日期格式**：YYYY.MM.DD，如 2026.03.28
- **段落简短**：每段 2-4 句，适合手机滑动阅读
- **强调标记**：只用「」包裹，不使用 `**文字**`（Markdown 加粗在小红书会显示为星号）
