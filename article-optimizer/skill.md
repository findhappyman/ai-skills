...
- 输出纯文本格式（不是 HTML）
- 小标题用「」包裹
- **⚠️ 禁止使用 `**文字**` 加粗标记**：小红书不支持 Markdown 语法，`**` 会原样显示为星号。需要强调时只用「」
- 引用用全角竖线 `｜` 开头
...
这是一条**贯穿整个技能**的硬性规则。无论是生成 Markdown 文件、微信公众号 HTML、X Article 文件、还是小红书纯文本，中文正文内容里的标点必须全部使用全角中文标点，严禁出现半角英文标点。
...
如果不放心，可以用以下 Python 脚本在输出文件上跑一遍做校验/修正：
