...
description: "优化中文文章并生成适配微信公众号、X（推特）、小红书三个平台的版本。 当用户要求\"优化文章\"、\"改写文章\"、\"帮我润色\"、\"发公众号/小红书/推特\"、 \"把这篇文章改成适合社交媒体发布的版本\"、\"帮我生成多平台内容\"时触发此技能。 也适用于用户直接粘贴一篇文章并要求修改、改写、优化、或适配不同平台的情况。 即使用户只提到其中一个平台，也应使用此技能，因为它包含完整的多平台适配逻辑。"
---
...
3. 使用 `present_files` 工具将文件提供给用户下载
4. 文件命名格式：`[主题关键词]_优化版.md`（如 `AI未来_优化版.md`）
